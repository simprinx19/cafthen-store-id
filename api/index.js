...
var SUPABASE_ID = "mgsqkkdjytqzodzmhwnv";
var SUPABASE_URL = process.env.SUPABASE_URL || `https://${SUPABASE_ID}.supabase.co`;
var DB_NAME = process.env.SUPABASE_DB_NAME || "db_cip";
var TABLE_NAME = process.env.SUPABASE_TABLE || "app_storage";
var SUPABASE_KEY = process.env.SUPABASE_ANON_KEY || process.env.SUPABASE_KEY || process.env.SUPABASE_SERVICE_ROLE_KEY || "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im1nc3Fra2RqeXRxem9kem1od252Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDAzNTA0MDAsImV4cCI6MjA1NTkyNjQwMH0.placeholder";
var globalWithSupabase = global;
...
var DEFAULT_CIP_LOGO = LOGO_PRESETS[0].svgDataUrl;

// src/mockData.ts
var INITIAL_PAYMENT_SETTINGS = {
  methods: [
    {
      id: "method-50-50",
      code: "50:50",
...
var INITIAL_PRODUCTS = [
  ...
  },
  {
    ...
    specs: [
      ...
      "Toleransi Dimensi: \xB10.3mm sesuai regulasi SNI",
  ...
  },
  {
    ...
    specs: [
      ...
      "Kuat Tekan 28 Hari: > 350 kg/cm\xB2",
  ...
  },
  {
    ...
    specs: [
      ...
      "Specific Gravity: > 2.55 gr/cm\xB3",
  ...
  },
  {
    ...
    specs: [
      "Tipe: Hydraulic Excavator 20 Ton (Bucket 0.93 - 1.0 m\xB3)",
  ...
  },
  {
    ...
    contract: {
      ...
      orderDetails: {
        ...
        unitPriceUSD: 0.77,
        subtotalIDR: 1875e5,
        taxAmountIDR: 20625e3,
        taxType: "Include PPN 11% (Jalur Darat)",
        shippingCostIDR: 0,
        totalAmountIDR: 208125e3,
...
var DATA_FILE = path.join(process.cwd(), "data.json");
var inMemoryCache = {};
function safeWriteLocalCache(data) {
...
var INITIAL_VALUES_MAP = {
  cafthen_company_profile: INITIAL_COMPANY_PROFILE,
  cafthen_products: INITIAL_PRODUCTS,
  cafthen_team_members: INITIAL_TEAM_MEMBERS,
  cafthen_activities: INITIAL_ACTIVITY_PHOTOS,
  cafthen_users: INITIAL_USERS,
  cafthen_orders: INITIAL_ORDERS,
  cafthen_expenses: INITIAL_EXPENSES,
  cafthen_messages: INITIAL_MESSAGES,
  cafthen_notifications: INITIAL_NOTIFICATIONS,
  cafthen_payment_settings: INITIAL_PAYMENT_SETTINGS,
  cafthen_theme_settings: INITIAL_THEME_SETTINGS,
  cafthen_exchange_rate: 17685
};
async function syncWithSupabaseData() {
...
var handleHealth = async (req, res) => {
  ...
  try {
    ...
    if (error) {
      ...
      sampleKeys = data ? data.map((d) => d.key) : [];
    }
  } catch (e) {
...
app.post("/api/db-test", async (req, res) => {
  ...
  const testPayload = { test: true, timestamp: (/* @__PURE__ */ new Date()).toISOString() };
  try {
    const supabase = getSupabaseClient();
    const t0 = Date.now();
    const { error: writeErr } = await supabase.from(TABLE_NAME).upsert({ key: testKey, value: testPayload, updated_at: (/* @__PURE__ */ new Date()).toISOString() }, { onConflict: "key" });
...
var handlePostData = async (req, res) => {
  try {
    ...
    const supabase = getSupabaseClient();
    if (key) {
      const { error } = await supabase.from(TABLE_NAME).upsert({ key, value, updated_at: (/* @__PURE__ */ new Date()).toISOString() }, { onConflict: "key" });
...
var handleResetDb = async (req, res) => {
  try {
    const supabase = getSupabaseClient();
    try {
